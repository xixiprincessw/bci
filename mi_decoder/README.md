...
4. **方法之间差 ≤0.06，人与人之间差 0.4~0.87**：目标被试自己的 ERD 强度（`mu_contra_pct`）与迁移精度 r=−0.58；6 个无 ERD 的被试，所有方法都在 0.51。跨被试的天花板由目标被试的信号质量决定，不由算法决定。
...
| `plot_article.py` | 从已有 csv 画文章配图（ERD-准确率散点 / 因果-零相位滤波 / 伪在线 CSP vs EEGNet 曲线） | 不重跑解码 |
...
- **6 人 none_or_reversed（占测试集 1/3）所有方法都在 0.51**：被试内 0.52、E1/E5/E4 0.51。不是迁移失败，是没有可迁移的信号。
